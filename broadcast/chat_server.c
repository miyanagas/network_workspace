...
static void client_login(
    int sock); /* クライアントの情報を記録する（ログイン）関数 */
static int receive_message(
    int sock,
    client_info *post_client); /* クライアントからのメッセージを受信する関数 */
static void client_logout(client_info *before_logout_client,
                          client_info *logout_client); /* クライアントの情報を抹消する（ログアウト）関数
                                                        */
static void send_message(
    char *mesg, char *username,
    client_info *post_client); /* クライアントにメッセージを送信する関数 */
...
void chat_server(in_port_t port_number, char *my_username) {
    ...
    for (;;) {
        ...
        if (FD_ISSET(udp_sock, &readfds)) {
            ...
            strsize = Recvfrom(udp_sock, buf, 4, 0,
                               (struct sockaddr *)&from_adrs, &from_len);
            buf[strsize] = '\0';
            if (strcmp(buf, "HELO") == 0) {
                /* HEREパケットの送信 */
                Sendto(udp_sock, "HERE", 4, 0, (struct sockaddr *)&from_adrs,
                       sizeof(from_adrs));
            }
        }
        ...
        for (before_ptr = Client_header, after_ptr = Client_header->next;
             after_ptr != NULL;
             before_ptr = after_ptr, after_ptr = after_ptr->next) {
...
static void send_message(char *mesg, char *username, client_info *post_client) {
    ...
    for (before_ptr = Client_header, after_ptr = Client_header->next;
        ...
        if (after_ptr != post_client) {
            if (send(after_ptr->sock, send_mesg, strlen(send_mesg),
                     MSG_NOSIGNAL) == -1) {
                if (errno == EPIPE) {
                    /* 接続が切れたクライアント情報の削除 */
